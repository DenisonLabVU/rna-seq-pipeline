...
report = pd.DataFrame()
sample_list = [line.rstrip('\n') for line in open("/Users/jennifergribble/Dropbox/NHC_recombination/SARS2_Calu/Recombination/SARS2_Calu_samples.txt")]
report['sample'] = sample_list
#Set other variables
virus = "SARS2"
wd = "/Users/jennifergribble/Dropbox/NHC_recombination/SARS2_Calu/Recombination/"
od = wd
exp = "SARS2_Calu_031521"
...
if Shannon_Entropy == True:
    ...
    # else:
    #     Min_Coverage = 0
    se_dir = "/Users/jennifergribble/Dropbox/NHC_recombination/SARS2_Calu/Recombination/Shannon_Entropy/"
    Virus_Accession = "MT020881.1"
    # se_output = pd.DataFrame(columns=['sample', Virus_Accession + "_to_" + Virus_Accession, Virus_Accession + "_RevStrand_to_" + Virus_Accession, Virus_Accession + "_RevStrand_to_" + Virus_Accession + "_RevStrand", Virus_Accession + "_to_" + Virus_Accession + "_RevStrand"])
        ...
        if fnmatch.fnmatch(file, "*_Virus_Recombination_Results.txt"):
            ...
            for Gene in Dicts:
                ...
                if Virus_Accession in Gene:
                    coverage_file = pd.read_csv(wd + sample_name + "_bbmap_coverage.txt", sep = "\t", header=0)
...
for file in os.listdir(wd + "Junction_Files/"):
    if fnmatch.fnmatch(file, "*forward_junctions.txt"):
        sample_name = file.split("_")[0]
        ...
        if (virus == "MHV"):
            ...
            DVGs = forward_junctions.loc[((forward_junctions['start_type'] == "DVG") | (forward_junctions['stop_type'] == "DVG"))]
        ...
        if (virus == "MERS"):
            ...
            sgmRNA_alternative = sgmRNAs.merge(sgmRNA_canonical, on=['genome', 'start', 'stop', 'type', 'depth', 'strand', 'rgb1', 'rgb2', 'start_seq', 'stop_seq', 'start_type', 'stop_type', 'frequency', 'logfreq'], how="left", indicator=True)
            sgmRNA_alternative = sgmRNA_alternative[sgmRNA_alternative["_merge"] == "left_only"]
            del sgmRNA_alternative["_merge"]
            sgmRNA_alt_summary = pd.DataFrame(columns=['type', 'depth'])
            ...
            DVGs = forward_junctions.loc[
                ((forward_junctions['start_type'] == "DVG") | (forward_junctions['stop_type'] == "DVG"))]
            DVGs.to_csv(save_dir_sgmRNAs + sample_name + "_DVGs.txt", sep="\t", index=False)
        ...
        if (virus == "SARS2"):
            ...
            forward_junctions['stop_type'] = forward_junctions['stop'].apply(
                lambda x: "sgmRNA2" if ((x >= 21526) & (x <= 21592)) else (
                                    ...
                                    "sgmRNA6" if ((x >= 27011) & (x <= 27076)) else(
                                        "sgmRNA7" if ((x >= 27358) & (x <= 27423)) else(
                                            "sgmRNA8" if ((x >= 27858) & (x <= 27923)) else(
                                                "sgmRNA9" if ((x >= 28230) & (x <= 28925)) else "DVG"
                                            )
                                        )
                                    )
                                )
            ...
            del sgmRNA_alternative["_merge"]
            sgmRNA_alt_summary = pd.DataFrame(columns=['type', 'depth'])
            sgmRNA_alt_summary['type'] = ['sgmRNA2', 'sgmRNA3', 'sgmRNA4', 'sgmRNA5', 'sgmRNA6', 'sgmRNA7', 'sgmRNA8', 'sgmRNA9']
            sgmRNA2_alt = sgmRNA_alternative.loc[sgmRNA_alternative['stop_type'] == "sgmRNA2"]
            sgmRNA3_alt = sgmRNA_alternative.loc[sgmRNA_alternative['stop_type'] == "sgmRNA3"]
            sgmRNA4_alt = sgmRNA_alternative.loc[sgmRNA_alternative['stop_type'] == "sgmRNA4"]
            sgmRNA5_alt = sgmRNA_alternative.loc[sgmRNA_alternative['stop_type'] == "sgmRNA5"]
...
sgmRNA_report['minor_sgmRNA_jfreq'] = (sgmRNA_report['minor_sgmRNA_depth'] / sgmRNA_report['total_nts']) * 100
